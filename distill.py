...
def nearest_neighbor(sentences, query_embeddings, database_embeddings):
    ...
    Returns:
      List of nearest sentences for each query.
    """
    nearest_neighbors = []
...
def get_images_texts(n, dataset):
    ...
    Returns:
      Tuple: (images tensor, texts tensor)
    """
...
def create_dataset(args, min_scale=0.5):
    # Use default image_size if missing.
...
def main(args):
    ...
    # Get synthetic images and texts.
    image_syn, text_syn = get_images_texts(args.num_queries, train_dataset)
    
    # Set synthetic data to require gradients.
