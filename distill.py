...
###############################################
# Main Buffer Distillation Script & Training Pipeline
###############################################
...
def main(args):
    # Print message for ROCO dataset.
    ...
    # Obtain synthetic images and texts.
    image_syn, text_syn = get_images_texts(args.num_queries, train_dataset)
    
    # Prepare the synthetic data (set requires_grad=True) and initialize optimizers.
    ...
    # Begin the distillation iterations.
                    ...
                    if args.zca:
                        ...
                        for clip_val in [2.5]:
                            ...
                            if args.dataset != "ImageNet":
                                ...
                                upsampled = torch.repeat_interleave(upsampled, repeats=4, dim=3)
                            grid = torchvision.utils.make_grid(upsampled, nrow=10, normalize=True, scale_each=True)
                            wandb.log({"Clipped_Reconstructed_Images/std_{}".format(clip_val): wandb.Image(torch.nan_to_num(grid.detach().cpu()))}, step=it)
                    if args.draw:
...
if __name__ == '__main__':
    ...
    # Additional arguments required for synthetic pipeline.
    ...
    parser.add_argument('--syn_steps', type=int, default=20, help='Number of synthetic steps per iteration')
    parser.add_argument('--mini_batch_size', type=int, default=100, help='Mini batch size for synthetic data')
    parser.add_argument('--max_start_epoch', type=int, default=25, help='Maximum index of starting expert epoch')
    ...
    parser.add_argument('--draw', type=bool, default=True, help='Whether to save drawn images')
